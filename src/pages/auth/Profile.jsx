...
const Profile = () => {
  ...
  const { changeUserData, user, userTheme, userMsg } = useContext(UserContext);
...
//  console.log(user_temp)
  ...
  return (
    <>
      <h1 className={'color'+nameAddColorCss}>{languagePack[lang]['PROFILE_OF']} {user?user.role==1?languagePack[lang]['TEACHER']:user.role==2?languagePack[lang]['MANAGER']:languagePack[lang]['STUDENT']:''}</h1>
      <div className={'bgText color'+nameAddColorCss+'Bg'}>
        ...
        >
          <Form.Item label={languagePack[lang]['USERNAME']}>
            <b>{user_temp.login}</b>
          </Form.Item>
          ...
          <Form.Item>
            <Button type="primary" onClick={changeUser}>{languagePack[lang]['SAVE']}</Button>
